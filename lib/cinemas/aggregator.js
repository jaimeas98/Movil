...
function logCinema(cinema, isoDates) {
  ...
  if (cinema.source === 'stale') {
    const minutos = Math.round((Date.now() - cinema.staleAt) / 60000);
    console.warn(`[cartelera] 🕓 ${cinema.id}: sirviendo datos de hace ${minutos} min — ${cinema.reason}`);
    return;
  }
...
// Última respuesta BUENA de cada cine. Cuando un cine deja de responder —Yelmo
// puso Cloudflare delante de su API y bloquea a nuestro servidor— enseñar lo
// último que sí conseguimos, avisando de que es antiguo, es mucho mejor que
// dejar el cine vacío: los horarios del día ya publicados no cambian.
const _ultimoBueno = new Map(); // cinemaId -> { at, byDate, liveDates }
const VIDA_RESPALDO_MS = 12 * 60 * 60 * 1000; // 12 h
...
async function getCinema(cinema, isoDates) {
  ...
  try {
    ...
    _ultimoBueno.set(cinema.id, { at: Date.now(), byDate, liveDates });
  } catch (err) {
    source = 'error';
    reason = String(err && err.message ? err.message : err);

    // Antes de darlo por perdido, recurrimos a la última respuesta buena. Los
    // horarios ya publicados de un día no cambian, así que enseñarlos avisando
    // de que son antiguos es más útil que dejar el cine en blanco. Se marca
    // como 'stale' para que la interfaz pueda decirlo, nunca como si fuera
    // información fresca.
    const respaldo = _ultimoBueno.get(cinema.id);
    if (respaldo && Date.now() - respaldo.at < VIDA_RESPALDO_MS) {
      for (const [iso, movies] of Object.entries(respaldo.byDate)) byDate[iso] = movies;
      liveDates = [...respaldo.liveDates];
      source = 'stale';
      staleAt = respaldo.at;
    }
