...
async function getCinema(cinema, isoDates) {
  ...
  let reason = null;
  let byDate = {};
  let liveDates = []; // fechas con datos reales del adaptador (sin muestra)
  ...
  try {
    ...
    if (live && Object.keys(live).length) {
      ...
      liveDates = Object.keys(live).sort();
  ...
  return {
    ...
    source,
    reason,
    liveDates, // fechas con datos reales (vacío si source='sample')
    byDate,
  };
}
...
export async function getShowtimes() {
  ...
  // Segunda pasada de seguridad: si algún cine×día quedó sin películas (por
  // cualquier motivo), se rellena con datos de muestra antes de cachear.
  for (const cinema of cinemas) {
    const sampleFill = buildSampleByDate(cinema.id, isoDates);
    for (const iso of isoDates) {
      if (!cinema.byDate[iso]?.length) {
        cinema.byDate[iso] = sampleFill[iso];
      }
    }
  }

  // Campo de diagnóstico: cuenta de películas por cine×día para detectar huecos.
  const _coverage = {};
  for (const cinema of cinemas) {
    _coverage[cinema.id] = {};
    for (const iso of isoDates) {
