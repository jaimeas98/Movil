...
// Log estructurado del servidor: ayuda a entender qué cine respondió, con cuántas
// películas y en cuántos días, o por qué falló. Visible en los logs de Vercel.
function logCinema(cinema) {
  const days = cinema.liveDates?.length ?? 0;
  const total = Object.values(cinema.byDate ?? {}).reduce((n, arr) => n + (arr?.length ?? 0), 0);
  if (cinema.source === 'live') {
    console.log(`[cartelera] ✅ ${cinema.id}: ${total} sesiones-película en ${days} días`);
  } else if (cinema.source === 'sample') {
    console.log(`[cartelera] 🧪 ${cinema.id}: datos de EJEMPLO (CARTELERA_SAMPLE=1)`);
  } else {
    console.warn(`[cartelera] ❌ ${cinema.id}: sin datos — ${cinema.reason}`);
  }
}
