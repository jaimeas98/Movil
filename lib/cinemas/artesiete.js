...
// Solo para diagnóstico: devuelve las sesiones tal cual vienen del HTML,
// sin procesar, para poder inspeccionar los nombres de campo reales
// (p.ej. verificar cómo se llama el ID de sesión/espectáculo para las
// URLs de compra) desde /api/diag.
export async function fetchArteSieteRawSample(limit = 2, titleFilter = null) {
  const html = await fetchText(CINE_URL, { headers: EXTRA_HEADERS });
  const sessions = extractAllSessions(html) || [];

  const stats = {
    totalSessions: sessions.length,
    withIdPase: sessions.filter((s) => s.ID_Pase).length,
    withIdEspectaculo: sessions.filter((s) => s.ID_Espectaculo).length,
    missingIdPaseSample: sessions
      .filter((s) => !s.ID_Pase)
      .slice(0, 3)
      .map((s) => ({ Titulo: s.Titulo, ID_Pase: s.ID_Pase, ID_Espectaculo: s.ID_Espectaculo })),
  };

  const pool = titleFilter
    ? sessions.filter((s) => String(s.Titulo || '').toLowerCase().includes(titleFilter.toLowerCase()))
    : sessions;

  return { stats, sample: pool.slice(0, limit) };
}
...
export async function fetchArteSiete() {
  ...
  for (const s of sessions) {
    ...
    const buyUrl = (s.ID_Pase && s.ID_Espectaculo)
      ? `https://bahia.artesiete.es/Session/33/ARTESIETE%20BAHIA/${s.ID_Pase}/${encodeURIComponent((s.Titulo || '').toUpperCase())}/${s.ID_Espectaculo}`
      : null;
    movie.sessions.push({
