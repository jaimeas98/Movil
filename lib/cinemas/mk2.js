...
export async function fetchMk2() {
  ...
  // Paso 2: para cada película, sesiones por día
...
function extractSessionsByDate(html, todayMadrid) {
  ...
  if (segments.length) {
    // Caso normal: cada segmento de contenido pertenece a un día.
    ...
    // Solo un día: toda la página es de ese día.
    const iso = dayDates[numKeys[0]];
    const sessions = extractBahiaSessions(html);
    if (sessions.length) result[iso] = sessions;
  }
