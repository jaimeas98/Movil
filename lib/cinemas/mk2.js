...
// Solo para diagnóstico: para cada segmento de día de la cartelera base,
// muestra el data-num, el texto real del rótulo de día, la fecha ISO que
// calcularíamos con addDays(hoy, num) vs. la fecha ISO real leída del rótulo,
// y (si se pasa titleFilter) los <a href> SIN recortar de las sesiones de esa
// película — para depurar por qué una sesión puede llevar a una fecha de
// compra distinta a la mostrada.
export async function fetchMk2RawSample(titleFilter = null) {
  const todayMadrid = madridToday();
  const cartHtml = await fetchText(CARTELERA_URL);
  const segments = splitDaySegments(cartHtml);
  const pool = segments.length ? segments : [{ num: '0', html: cartHtml }];

  const out = [];
  for (const { num, html: seg } of pool) {
    const rotuloM = seg.match(/rotulo_dia[^>]*>([^<]+)</);
    const rotuloText = rotuloM ? rotuloM[1].trim() : null;
    const isoByNum = addDays(todayMadrid, parseInt(num, 10) || 0);
    const isoByRotulo = isoFromSegment(seg, todayMadrid);

    const movies = [];
    for (const block of seg.split('<div class="film-list-item">').slice(1)) {
      const linkM = block.match(/href="https:\/\/www\.mk2cines\.es\/es\/([a-z0-9-]+)"[^>]*class="negro"[^>]*>([^<]+)<\/a>/);
      if (!linkM) continue;
      const title = linkM[2].trim();
      if (titleFilter && !title.toLowerCase().includes(titleFilter.toLowerCase())) continue;

      const sessions = [];
      for (const horariosBlock of block.split('<div class="horarios">').slice(1)) {
        if (!horariosBlock.includes(CINEMA_SLUG)) continue;
        for (const a of horariosBlock.matchAll(
          /<a[^>]+href="([^"]*cinesur-bahia-de-cadiz[^"]*)"[^>]*>([\s\S]*?)<\/a>/g
        )) {
          sessions.push({
            rawHref: a[1].replace(/&amp;/g, '&'),
            text: a[2].replace(/<[^>]+>/g, '').trim(),
          });
        }
      }
      if (sessions.length) movies.push({ title, sessions });
    }

    if (movies.length || !titleFilter) {
      out.push({ num, rotuloText, isoByNum, isoByRotulo, movies });
    }
  }
  return out;
}
...
function extractSessionsByDateFromMoviePage(html, todayMadrid) {
  ...
  if (segments.length > 1) {
    ...
    for (const { num, html: seg } of segments) {
      ...
      if (!sessions.length) continue;
      // En mk2 el `data-num="0"` NO corresponde necesariamente a hoy: si el cine
      // no abre hoy, num=0 es el primer día con programación. Por eso intentamos
      // primero leer la fecha REAL del título del rotulo_dia (ej. "Jueves 04/06")
      // y solo si no la encontramos caemos a addDays como último recurso.
      const iso = isoFromSegment(seg, todayMadrid) || addDays(todayMadrid, parseInt(num, 10) || 0);
...
function isoFromSegment(seg, todayIso) {
  const m = seg.match(/rotulo_dia[^>]*>([^<]+)</);
  if (!m) return null;
  const dm = m[1].match(/(\d{1,2})\/(\d{1,2})/);
  if (!dm) return null;
  const day = String(dm[1]).padStart(2, '0');
  const month = String(dm[2]).padStart(2, '0');
  const [yearStr, todayMonthStr] = todayIso.split('-');
  let year = Number(yearStr);
  if (Number(month) < Number(todayMonthStr) - 6) year += 1;
