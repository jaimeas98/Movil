...
// Días hacia adelante que intentamos obtener además de hoy.
const EXTRA_DAYS = 6;
...
export async function fetchMk2() {
  ...
  // Paso 2: para cada película, intentar obtener sesiones de varios días.
  // La página individual muestra hoy por defecto (HTML estático); los días
  // siguientes se renderizan con JS via tab "data-num". Probamos el parámetro
  // de URL que usa el CMS para forzar un día concreto.
  const perMovie = await Promise.all(
    movies.map(async (movie) => {
      const sessionsByDate = {};
      let genre = movie.genre;

      // Hoy (siempre funciona; también extraemos el género aquí)
      try {
        const html = await fetchText(`${BASE}/es/${movie._slug}`, { timeoutMs: 9000 });
        const todaySessions = extractBahiaSessions(html);
        if (todaySessions.length) sessionsByDate[todayMadrid] = todaySessions;
        const g = extractGenreFromMoviePage(html);
        if (g) genre = g;
      } catch { /* ignorar, el agregador rellena con muestra */ }

      // Días siguientes: probamos parámetros comunes del CMS mk2/Cinemana
      for (let dayOffset = 1; dayOffset <= EXTRA_DAYS; dayOffset++) {
        const iso = addDays(todayMadrid, dayOffset);
        // Los tabs usan data-num="1", "2"… → parámetro ?dia=1, ?id_dia=1, etc.
        const urls = [
          `${BASE}/es/${movie._slug}?dia=${dayOffset}`,
          `${BASE}/es/${movie._slug}?id_dia=${dayOffset}`,
          `${BASE}/es/${movie._slug}?day=${dayOffset}`,
        ];
        for (const url of urls) {
          try {
            ...
            if (sessions.length) {
              sessionsByDate[iso] = sessions;
              break; // URL que funciona encontrada; pasar al día siguiente
            }
          } catch { /* URL no válida, probar la siguiente */ }
        }
      }

      return { movie: { ...movie, genre }, sessionsByDate };
    })
  );
...
function extractSessionsByDate(html, todayMadrid) {
  // Las páginas de película de mk2 renderizan las sesiones del día activo
  // por defecto (hoy). Los tabs de días futuros son JS-interactivos y no
  // están en el HTML estático. Extraemos todo lo visible y lo asignamos a hoy.
  const sessions = extractBahiaSessions(html);
