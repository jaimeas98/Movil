...
import instantanea from '../../data/yelmo.js';
...
export async function fetchYelmo(cinemaId) {
  ...
  try {
    ...
    // Extraer el ID de Vista del cine (campo según versión del API)
    const cinemaVistaId = cinema.VistaId ?? cinema.CinemaVistaId ?? cinema.Id ?? null;

    dates.forEach((day, idx) => {
      const iso = dateOfDay(day, idx, todayStr);
      if (!iso) return;
      const movies = (day.Movies ?? []).map((m) => parseYelmoMovie(m, cinemaVistaId));
  ...
  // ── 4. Instantánea recogida desde una máquina doméstica ────────────────────
  // Yelmo bloquea a cualquier servidor, así que los tres caminos de arriba
  // pueden fallar los tres a la vez y no por un fallo nuestro. Cuando eso pasa,
  // usamos lo que haya recogido scripts/recolector-yelmo.mjs desde un equipo
  // normal, que a él sí le responden. Se guarda tal cual viene de su API, de
  // modo que lo interpreta este mismo parser: no hay dos formatos que mantener.
  if (!Object.keys(byDate).length) {
    const desdeInstantanea = leerInstantanea(cinemaId);
    if (desdeInstantanea) return desdeInstantanea;
  }
...
// Edad máxima de la instantánea. Más allá deja de ser información y pasa a ser
// ruido: mejor que el cine salga vacío y se vea, a enseñar la cartelera de la
// semana pasada como si fuera la de hoy.
const VIDA_INSTANTANEA_MS = 36 * 60 * 60 * 1000;

// `datos` se puede inyectar para poder probar esto sin tocar el fichero ni
// pelearse con la caché de módulos.
export function leerInstantanea(cinemaId, datos = instantanea) {
  const cine = datos?.cines?.[cinemaId];
  if (!cine || !datos?.generatedAt) return null;

  const edad = Date.now() - new Date(datos.generatedAt).getTime();
  if (!(edad >= 0) || edad > VIDA_INSTANTANEA_MS) return null;

  const todayStr = msToMadridDate(Date.now());
  const cinemaVistaId = cine.VistaId ?? cine.CinemaVistaId ?? cine.Id ?? null;
  const byDate = {};
  (cine.Dates ?? []).forEach((day, idx) => {
    const iso = dateOfDay(day, idx, todayStr);
    if (!iso) return;
    // Los días ya pasados de una instantánea de ayer no aportan nada.
...
function parseYelmoMovie(f, cinemaVistaId = null) {
  ...
  for (const fmt of f.Formats ?? []) {
    ...
    for (const s of fmt.Showtimes ?? []) {
      let buyUrl = null;
      if (s.ShowtimeId) {
        buyUrl = cinemaVistaId
          ? `https://compra.yelmocines.es/?cinemaVistaId=${cinemaVistaId}&showtimeVistaId=${s.ShowtimeId}`
          : `https://compra.yelmocines.es/?showtimeVistaId=${s.ShowtimeId}`;
      }
      sessions.push({
