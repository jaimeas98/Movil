...
function dateOfDay(day, idx, todayStr) {
  ...
  // Formato actual: "DD/MM/YYYY HH:MM:SS" (en hora local de España).
  const dm = tf.match(/^(\d{2})\/(\d{2})\/(\d{4})/);
  if (dm) return `${dm[3]}-${dm[2]}-${dm[1]}`;
  // Formato legado por si alguna respuesta vieja aún lo trae: "/Date(ms)/".
  const ms = Number((tf.match(/\/Date\((\d+)\)\//) || [])[1]);
  if (!isNaN(ms) && ms > 0) return msToMadridDate(ms);
