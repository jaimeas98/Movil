...
import { normalizaGenero } from './normalize.js';
...
export function calcularProximas(cinemas, ultimoDiaVisible, valoraciones = {}) {
  ...
  for (const cine of cinemas ?? []) {
    for (const [iso, peliculas] of Object.entries(cine.byDate ?? {})) {
      ...
      for (const peli of peliculas ?? []) {
        ...
        // El cartel de TMDB manda, igual que en la cartelera: los cines los
        // sirven a resoluciones muy dispares.
        const mejorCartel = valoraciones?.[peli.title]?.poster || peli.posterUrl;
        if (!ficha.posterUrl && mejorCartel) ficha.posterUrl = mejorCartel;
        if (!ficha.genre && peli.genre) ficha.genre = normalizaGenero(peli.genre);
